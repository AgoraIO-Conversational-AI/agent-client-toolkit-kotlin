...
Tap Start Agent
  ...
  → POST /join with top-level `preset` (managed ASR only)
  → save agentId
  → uiState = Connected
```
...
- no client-side `llm` block
- no client-side `tts` block

No third-party API key is required in the current flow.
